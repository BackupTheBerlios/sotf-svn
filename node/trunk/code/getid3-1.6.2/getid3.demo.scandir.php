...
if (!empty($_REQUEST['scan']) || !empty($_REQUEST['newscan']) || !empty($_REQUEST['rescanerrors'])) {
	...
	echo number_format($totalremoved).' of '.number_format($totalchecked).' files in database no longer exist - removed from database.<HR>';
	...
	if (!empty($_REQUEST['showtagfiles'])) {
		echo '<TABLE BORDER="1" CELLSPACING="0" CELLPADDING="3">';
