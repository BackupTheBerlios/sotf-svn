...
// array listing (<extension> => <filetype>) for assuming format
$formatExtensions = array('mp3'=>'mp3', 'mp2'=>'mp3', 'mp1'=>'mp3', 'ogg'=>'ogg', 'zip'=>'zip', 'wav'=>'riff', 'avi'=>'riff', 'mid'=>'midi', 'mpg'=>'mpeg', 'mpeg'=>'mpeg');
...
if (isset($_REQUEST['filename'])) {
	...
	if (isset($_REQUEST['assumeFormat'])) {
		...
		if (empty($ThisFileInfo['fileformat']) || ($ThisFileInfo['fileformat'] == 'id3')) {
			if (isset($formatExtensions[strtolower(fileextension($_REQUEST['filename']))])) {
				$ThisFileInfo = GetAllFileInfo($_REQUEST['filename'], $formatExtensions[strtolower(fileextension($_REQUEST['filename']))], $AutoGetMD5file, $AutoGetMD5data, true); // auto-get md5_data if filesize < 50MB
			}
		}
	}
		...
		foreach ($DirectoryContents as $dirname => $val) {
			...
			if (isset($DirectoryContents["$dirname"]['known']) && is_array($DirectoryContents["$dirname"]['known'])) {
				...
				foreach ($DirectoryContents["$dirname"]['known'] as $filename => $fileinfo) {
					...
					echo '<TD ALIGN="LEFT">&nbsp;';
					if (in_array('id3v1', $fileinfo['tags']) || in_array('id3v2', $fileinfo['tags']) || in_array('ape', $fileinfo['tags'])) {
