...
/*  
 * $Id: editMeta.php 577 2006-05-29 12:04:06Z buddhafly $
	  ...
	  for($q = 0; $q < count($audioFiles); $q++) {	    	        
		  ...
		  if($extension=="mp3") {	   
			...
			$tagwriter->tag_encoding   = $TaggingFormat;
			  ...
			  // TITLE	 
			  $tagdata['title'][]= getid3_lib::SafeStripSlashes(sotf_Utils::getParameter('title'));	// renamed $data to $tagdata to prevent conflict with $data in loadvars.inc.php
			  ...
			  $tagdata['artist'][]= getid3_lib::SafeStripSlashes($station_name);	
			  ...
			  $tagdata['album'][]= getid3_lib::SafeStripSlashes($series_name);
			  
			  
			  // YEAR	     
			     
			  $tagdata['year'][]= getid3_lib::SafeStripSlashes(sotf_Utils::getParameter('production_date' . 'Year'));
		   	        	        
			  // GENRE
			  $tagdata['genre'][] = getid3_lib::SafeStripSlashes($vocabularies->getGenreName($prg->get('genre_id')));
			  	        
	  
			  // COMMENT	
			  $tagdata['comment'][] = ' *ID3Tags modified by StreamOnTheFly*';
...
// added by wolfgang csacsinovits & martin schmidt 05-09-27	  
 ...
 elseif ($finishpublish) {
	...
	$page->redirect("get.php?id=$prg->id");
  } elseif ($finish) {
    $prg->update();
    	//$page->redirect("editor.php");
	$page->redirect("get.php?id=$prg->id");
    ...
    $prg->update();
    $page->redirect("editMeta.php?id=$prg->id&newsave=$new"); //new-parameter added by Klaus Temper
  }
}
...
$smarty->assign("NEWSAVE",sotf_Utils::getParameter('newsave'));
$page->send();
