...
/*======================================================================*\
    Function: smarty_mod_access
    Purpose:  check whether user has access
\*======================================================================*/
function smarty_mod_access($string, $allowed="")
{
	$allows = explode(",", $allowed);
	$spermissions = explode(",", $string);
	foreach ($spermissions as $compare) {
		if (in_array ($compare, $allows)) {
			return "yes";
		}
	}
    if ($user == $allowed) {
...
/*======================================================================*\
    Function: smarty_mod_boolean
    Purpose:  make a boolean. (for use in show parameter)
\*======================================================================*/
function smarty_mod_boolean($string)
{
	if (($string == "1") or ($string == "true") or ($string == "t")) {
		return "true";
		} else {
		return "";
		}
...
\*======================================================================*/
function smarty_mod_debug_print_var($var, $depth=0, $length=200) {
	if (is_array($var)) {
...
/*======================================================================*\
    Function: smarty_restrict_access
    Purpose:  get a username and a password or a logged-in user.
\*======================================================================*/

function smarty_restrict_access ($args, &$smarty_obj) {
/*	global $username;*/
	/*We need some DB access here.*/
	extract($args);
/*	$username = $smarty_obj->$username;*/
	$tpl_vars = $smarty_obj->get_template_vars();
	$username = $tpl_vars["username"];	
	$userlist = explode(",", $allowed);
	require_once 'DB.php'; 
	include('local.inc');

	$smarty_obj->clear_assign("access_granted");

	$db = DB::connect($dsn); 
	// Mit DB::isError kann man zwischen einem Fehler und 
	// einer korrekten verbindung unterscheiden. 
	if (DB::isError($db)) { 
			die ($db->getMessage()); 
	} 
	$query="select auth_id from authenticate where username ='" . $username . "'";
	$account_id = $db->getOne("$query");
	if (DB::isError($account_id)) { 
			$smarty_obj->assign("DBError", $account_id->getMessage() . " / $query"); 
			$smarty_obj->assign("account", $account_id); 
			$account_id = 0; 
	}
	$smarty_obj->assign("Debug_account_id", $account_id . " ($query)");

	/* An arbitrarily long list of user or groupnames can be passed to this function. */
	/* A user not in that list or a group on that list will be rejected.*/
	
	if (count($userlist) == 1) {
			if ($userlist[0] == "$username") {
				$smarty_obj->assign('access_granted', 'true');
				return;
			}
	} else {
	...
	$smarty_obj->assign('access_userlist', $userlist);
	/* Gruppencheck */
	if (count($userlist) > 1) {
	$ARGLIST = implode("','", $userlist);
	} else {
	$ARGLIST = $userlist[0];
	}
	$smarty_obj->assign('access_arglist', $ARGLIST);
	$query = "select count(*) from acc_permissions where account_id = $account_id and permission_name in ('" . $ARGLIST . "')  and is_allowed = true";
	$smarty_obj->assign('access_query', $query);
	$groupquery = $db->getOne("$query");
	if (DB::isError($groupquery)) { 
        $smarty_obj->assign('access_groupquery', ($groupquery->getMessage()) . $query);
        } else {
		$smarty_obj->assign('access_groupquery', "$groupquery");
		}
	if ($groupquery == 1) {
		 $smarty_obj->assign('access_granted', 'true');
		 return;
	}
	// if we get here, no check was successful, so we return false.
/*	$smarty_obj->assign('access_granted', 'false');*/
